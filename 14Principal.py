...
api_key = os.getenv('API_KEY')  # Reemplaza con tu clave de API de VirusTotal
...
def notify_admin(error_message):
    admin_email = 'rherrera.ciberseguridad@gmail.com'
...
def format_analysis_report(report):
    try:
        ...
        Estadísticas:
            Malicioso: {stats['malicioso']}
            Sospechoso: {stats['sospechoso']}
            No Detectado: {stats['no_detectado']}
            Inofensivo: {stats['Inofensivo']}
            Tiempo de Espera: {stats['tiempo_de_espera']}
            Tiempo de Espera Confirmado: {stats['tiempo_de_espera_confirmado']}
            Fallos: {stats['fallos']}
            Tipo No Soportado: {stats['NO_soportado']}
